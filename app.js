...
/* =========================================================
   CONFIG
   ========================================================= */
const MATCH_RADIUS_M = 90;          // how far around the user we look for shops
const RECHECK_MIN_DISTANCE_M = 35;  // re-query Overpass only after moving this far
const RECHECK_MIN_INTERVAL_MS = 25000; // ...or after this much time
...
function startLocating() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    (err) => {
      setStatus("Poloha nepovolena — vyber kartičku ručně", "err");
      refreshCards().then(() => {
        if (allCards.length === 0) renderForNoCards();
        else renderCardList(allCards, "Poloha nepovolena — vyber ručně");
      });
    },
...
/* =========================================================
   NAVIGATION BUTTONS
   ========================================================= */
els.btnAllFromMatch.addEventListener("click", () => renderCardList(allCards, "Všechny kartičky"));
els.btnAllFromChoice.addEventListener("click", () => renderCardList(allCards, "Všechny kartičky"));
els.btnBackFromView.addEventListener("click", () => renderCardList(allCards, "Všechny kartičky"));
...
(async function boot() {
  ...
  if (allCards.length === 0) {
    ...
    setStatus("Hledám, kde právě jsi…", "warn");
  }
  startLocating();
  maybeShowIosBanner();
})();
